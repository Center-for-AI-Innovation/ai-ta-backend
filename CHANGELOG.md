...
### Added
- Per-project embedding resolution in the ingest worker. `WorkerConnectionResolver.resolve()` now reads the `embedding_config` column (legacy fallback: `qdrant_config.embedding`) and carries `embedding_provider/model/api_key/api_base/base_url` on `ResolvedConnections`. The ingest worker embeds documents with the project's configured model — `provider=ollama` uses native LangChain `OllamaEmbeddings` (matching retrieval's `/api/embeddings` shape for consistent vectors); openai/no-config uses the existing OpenAI-compatible batch path. Provider is gated by `ALLOWED_EMBEDDING_PROVIDERS`; a disallowed/misconfigured project logs and falls back to the env default instead of crashing the worker.
- `ai_ta_backend/rabbitmq/.env.template` (committed) — a subset of the backend `.env` covering the worker/producer's default connections, plus a gitignored `.env` for local values.
...
### Fixed
- Ingest status tables (`documents_in_progress`, `documents_failed`) now live on the project's documents DB (external when `database_config` is set, host otherwise), matching where the frontend reads them via `connectionManager.getDocumentsDb`. The producer (`rmqueue.py`) and worker (`worker.py`) previously wrote/read these on the host DB while `ingest.py` used the project DB, orphaning in-progress rows and writing failures to the wrong DB for external-DB projects.
