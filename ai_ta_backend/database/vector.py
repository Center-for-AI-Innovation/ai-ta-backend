...
class VectorDatabase:
    ...
    def execute_search(self, query_filter, query_vector, course_name: str, top_n: int = 100):
        ...
        If qdrant_config contains a 'collections' array, dispatches to
        multi-collection parallel search with per-collection processors.
        Otherwise, performs a single-collection search using 'collection_name'.
        """
        collections = self.qdrant_config.get("collections")
        if collections:
            return self._multi_collection_search(
                collections, query_filter, query_vector, course_name, top_n
            )
        return self._single_collection_search(
            query_filter, query_vector, top_n
        )

    def _single_collection_search(self, query_filter, query_vector, top_n: int):
        """Standard search against a single Qdrant collection."""
        search_kwargs = dict(
