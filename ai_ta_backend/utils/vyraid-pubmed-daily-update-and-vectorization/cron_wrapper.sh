...
# 0 2 * * * /home/dadams/pub-med-daily/ai-ta-backend/ai_ta_backend/utils/vyraid-pubmed-daily-update-and-vectorization/cron_wrapper.sh >> /var/log/pipeline_cron.log 2>&1
...
if [ "$CONDA_ACTIVATED" -eq 1 ]; then
    ...
    PYTHON="/home/dadams/miniforge3/bin/python"
    if [ ! -x "$PYTHON" ]; then
