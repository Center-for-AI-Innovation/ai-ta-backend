...
def log_thread_metrics(logger=None, capture_callback=None):
  """Log thread metrics for monitoring.

    Args:
        logger: Optional logger to write human-readable line.
        capture_callback: Optional callable taking (event_name: str, properties: dict)
            used to emit structured metrics (e.g., PostHog capture).
    """
  info = get_thread_info()

  message = (f"Thread Monitor - "
  ...
  # Emit structured metrics if provided
  if capture_callback:
    try:
      system = info.get('system', {})
      capture_callback(
          'thread_monitor_heartbeat', {
              'pid': system.get('pid'),
              'num_threads': system.get('num_threads'),
              'memory_mb': system.get('memory_info_mb'),
              'cpu_percent': system.get('cpu_percent'),
              'python_thread_count': info.get('thread_count'),
              'executor_states': info.get('executor_states'),
          })
    except Exception as e:
      if logger:
        logger.warning(f"Failed to capture PostHog metrics: {e}")
