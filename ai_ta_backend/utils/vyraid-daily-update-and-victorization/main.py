...
class DB:
    ...
    def update_xml_log_progress(self, xml_filename: str, last_processed_pmcid: str, delta_seconds: float, newly_processed_count: int) -> None:
        ...
        total_pmcid_processed by newly_processed_count.
        """
        log_tbl = self._tbl("xml_processing_log")
        q = sql.SQL(
            """
...
def oa_fetch_pdf(session: requests.Session, pmcid: str) -> Optional[Tuple[str, str]]:
    ...
    for attempt in range(5):
        try:
            r = session.get(OA_API, params=params, timeout=60)
...
class ProcessStateLogger:
    ...
    def mark_completed(self, processed_count: int) -> None:
...
def run_pipeline(limit: Optional[int] = None) -> None:
    ...
    # Set Qdrant indexing threshold to 0 for bulk processing
    qdrant.set_indexing_threshold(0)
    time.sleep(2)
    ...
    for xml_fname in files:
        ...
        # download this XML.gz to a tempdir
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                for attempt in range(5):
                    try:
                        with http_session.get(UPDATE_URL + xml_fname, stream=True, timeout=60) as r:
                            r.raise_for_status()
            ...
            def _process_row_inner(row: ArticleRow):
                ...
                with tempfile.TemporaryDirectory() as file_tmp:
                    ...
                    for attempt in range(5):
                        try:
                            with http_session.get(download_url, stream=True, timeout=60) as r:
            ...
            log.info("Finished XML %s: %d ok%s", xml_fname, processed_in_file_ok, " (aborted early)" if aborted_early else "")
    ...
    # Reset Qdrant indexing threshold to normal operation value
