...
def _validate_connection_config(data: dict):
  """Validate required fields in each connection config block. Aborts with 400 on missing fields."""
  errors = []
  ...
  if data.get('s3_config'):
    ...
    for field in ('aws_access_key_id', 'aws_secret_access_key'):
      if not cfg.get(field):
        errors.append(f"s3_config.{field}")

  if data.get('database_config'):
    cfg = data['database_config']
    if not cfg.get('connection_uri'):
      errors.append("database_config.connection_uri")

  if data.get('qdrant_config'):
    cfg = data['qdrant_config']
    for field in ('url', 'api_key', 'port', 'default_collection'):
      if not cfg.get(field):
        errors.append(f"qdrant_config.{field}")
    if 'collections' in cfg and not isinstance(cfg['collections'], list):
      errors.append("qdrant_config.collections (must be an array)")

  if errors:
    abort(400, description=f"Missing required fields: {', '.join(errors)}")
...
def upsert_project_connection(sql: SQLDatabase, conn_manager: ConnectionManager) -> Response:
  ...
  project = sql.getProjectByName(project_name)
  if not project:
    abort(404, description=f"Project '{project_name}' not found. Create it first via /createProject.")
  project_id = project['id']

  # Validate required fields before encryption
  _validate_connection_config(data)
  ...
  result = sql.upsertExternalConnection(project_id, project_name, s3_config, database_config, qdrant_config)

  # Invalidate cached connections so next request uses the new config
  conn_manager.invalidate(project_name)

  response = jsonify({"success": True, "project_name": project_name, "project_id": project_id})
  response.headers.add('Access-Control-Allow-Origin', '*')
  return response
...
SUPPORTED_CONNECTION_TYPES = ('s3', 'database', 'qdrant')
...
def delete_project_connection(sql: SQLDatabase, conn_manager: ConnectionManager) -> Response:
  ...
  Query params:
    project_name (required) -- project to target.
    type (optional)        -- when omitted, deletes the entire row (legacy
                              behavior). When one of 's3'/'database'/'qdrant',
                              clears only that config field on the row.
  """
  project_name = request.args.get('project_name', default='', type=str)
  if not project_name:
    abort(400, description="Missing required parameter: 'project_name'")

  conn_type = request.args.get('type', default=None, type=str)
  if conn_type is not None and conn_type not in SUPPORTED_CONNECTION_TYPES:
    abort(
        400,
        description=(
            f"Invalid type: '{conn_type}'. Must be one of: "
            f"{', '.join(SUPPORTED_CONNECTION_TYPES)}. Omit 'type' to delete the entire row."
        ),
    )

  result = sql.deleteExternalConnection(project_name, conn_type)
  conn_manager.invalidate(project_name)
...
@app.route('/api/project-connections/active', methods=['PATCH'])
def set_project_connection_active(sql: SQLDatabase, conn_manager: ConnectionManager) -> Response:
  """Toggle the is_active flag on a project's external connection row.

  Request body: {"project_name": "...", "is_active": true|false}
  When is_active is set to false the project falls back to the shared default
  infrastructure without losing its stored configs.
  """
  ...
  if 'is_active' not in data:
    abort(400, description="Missing required field: 'is_active' (boolean)")
  is_active = data['is_active']
  if not isinstance(is_active, bool):
    abort(400, description="'is_active' must be a boolean")

  result = sql.setExternalConnectionActive(project_name, is_active)
  if not result['found']:
    abort(404, description=f"No external connection config found for project '{project_name}'.")
...
def test_project_connection() -> Response:
  ...
  if conn_type == 'database':
    ...
    missing = [f for f in ('url', 'api_key', 'port') if not config.get(f)]
    if missing:
      abort(400, description=f"Missing required fields in config: {', '.join(missing)}")
    ...
    missing = [f for f in ('aws_access_key_id', 'aws_secret_access_key') if not config.get(f)]
    if missing:
      abort(400, description=f"Missing required fields in config: {', '.join(missing)}")
