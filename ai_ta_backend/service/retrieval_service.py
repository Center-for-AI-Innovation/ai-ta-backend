...
class RetrievalService:
    ...
    async def getTopContexts(
        ...
        try:
            ...
            project_sql_db = self.conn_manager.get_sql_db(course_name)
    ...
    def delete_data(self, course_name: str, s3_path: str, source_url: str):
        ...
        try:
            # Delete file from S3 - use ConnectionManager for dynamic resolution
            ...
            logging.info(
                f"Deleting {identifier_value} from S3, Qdrant, and Database using {identifier_key}"
            )
    ...
    def delete_from_qdrant(
        ...
        try:
            ...
            vdb = self.conn_manager.get_vector_db(course_name)
            collection_name = vdb.qdrant_config["collection_name"]
            response = vdb.delete(collection_name, identifier_key, identifier_value)
    ...
    def _resolve_embedding_client(self, qdrant_config: dict):
        ...
        """
        embedding_cfg = qdrant_config.get("embedding")
    ...
    def getProjectStats(self, project_name: str) -> ProjectStats:
        ...
        """
        return self.conn_manager.get_sql_db(project_name).getProjectStats(project_name)
    ...
    def getWeeklyTrends(self, project_name: str) -> List[WeeklyMetric]:
        ...
        """
        return self.conn_manager.get_sql_db(project_name).getWeeklyTrends(project_name)
    ...
    def getModelUsageCounts(self, project_name: str) -> List[ModelUsage]:
        ...
        """
        try:
            return self.conn_manager.get_sql_db(project_name).getModelUsageCounts(
                project_name
            )
            ...
            for i, chunk in enumerate(chunks):
                try:
                    ...
                    doc = {
                        "id": str(
                            uuid.uuid4()
                        ),  # Use UUID instead of conversation_id_index
                        "vector": embedding,
                        "payload": {
                            "course_name": course_name,
                            "conversation_id": conversation_id,
